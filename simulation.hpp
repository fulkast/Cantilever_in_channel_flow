...
//		l.add_to_shapes(new quadrilateral_2D(lb::coordinate<double>(10*D-10,10*D-10), 0, D, D));
		l.add_to_shapes(new quadrilateral_cantilever_2D(lb::coordinate<double>(10*D,10*D), 0,10*D, D/10,10, 1));
	...
	void wall_bc()
	{
		...
		for (auto i = l.shapes.begin(); i != l.shapes.end(); i++)
		{
			...
			if (sim_type == "dynamic")
			{
				...
				(*i)->create_sinusoidal_motion(0.1*sin(time*2*3.141596265/1000)*0.5);
